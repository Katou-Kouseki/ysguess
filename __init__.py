...
sv = Service('原神猜', help_='猜头像''''
原神猜 | 猜猜派蒙随机发送的头像的一小部分来自哪位角色
'''.strip())
...
async def ys_guess(bot, ev: CQEvent):
    try:
        ...
        chara_id_list = list(data.CHARA_NAME.keys())
        while True:
        ...
        file_path = os.path.join(dir_path, 'cropped_snqx.png')
        cropped.save(file_path)
        image = MessageSegment.image(f'file:///{os.path.abspath(file_path)}')
        msg = f'猜猜这个图片是游戏中谁的一部分?({ONE_TURN_TIME}s后公布答案){image}'
    ...
    try:
        if winner_judger.get_on_off_status(ev.group_id):
            ...
            if cid != chara.UNKNOWN and cid == winner_judger.get_correct_chara_id(
                ...
                c = YsChara(winner_judger.get_correct_chara_id(ev.group_id))
                msg = f'正确答案是: {c.name}{c.icon.cqcode}\n{msg_part}'
                await bot.send(ev, msg)
    except Exception as e:
        await bot.send(ev, '错误:\n' + str(e))
